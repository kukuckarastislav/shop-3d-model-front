...
export class PurchasesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.productService.GetMyPurchases().subscribe((data) => {
      this.purchases = data;
    } );
  }
  ...
  redirectUserPage(purchase: Purchase) {
    window.location.href = '/user/products/' + purchase.seller.uuid;
  }
  ...
  showPurchaseIf(purchase: Purchase) {
    ...
    if (this.searchUserName != '' && !purchase.seller.name.toLowerCase().includes(this.searchUserName.toLowerCase())) {
      return false;
    }
